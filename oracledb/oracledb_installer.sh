...
setup_oracledb()
{
    ...
    echo "Enter tags you want to add to this Oracle DB(optional): "
...
setup_standard_init()
{
    ...
    INIT_FILE="/etc/init.d/revealmetrics_oracledb"
    if [ -e "$INIT_FILE" ]; then
...
ENDINIT
    ...
    if [ -n "$CREATED_INIT" ]; then
        ...
        if [ -d "/etc/rc1.d" ]; then
            rm -f /etc/rc*.d/*revealmetrics_oracledb
            ln -s $INIT_FILE /etc/rc0.d/K99revealmetrics_oracledb
            ln -s $INIT_FILE /etc/rc1.d/K99revealmetrics_oracledb
            ln -s $INIT_FILE /etc/rc2.d/S99revealmetrics_oracledb
            ln -s $INIT_FILE /etc/rc3.d/S99revealmetrics_oracledb
            ln -s $INIT_FILE /etc/rc4.d/S99revealmetrics_oracledb
            ln -s $INIT_FILE /etc/rc5.d/S99revealmetrics_oracledb
            ln -s $INIT_FILE /etc/rc6.d/K99revealmetrics_oracledb
        elif [ -d "/etc/init.d/rc1.d" ]; then
            rm -f /etc/init.d/rc*.d/*revealmetrics_oracledb
            ln -s $INIT_FILE /etc/init.d/rc1.d/K99revealmetrics_oracledb
            ln -s $INIT_FILE /etc/init.d/rc2.d/S99revealmetrics_oracledb
            ln -s $INIT_FILE /etc/init.d/rc3.d/S99revealmetrics_oracledb
            ln -s $INIT_FILE /etc/init.d/rc4.d/S99revealmetrics_oracledb
            ln -s $INIT_FILE /etc/init.d/rc5.d/S99revealmetrics_oracledb
            ln -s $INIT_FILE /etc/init.d/rc6.d/K99revealmetrics_oracledb
            ln -s $INIT_FILE /etc/init.d/rcS.d/S99revealmetrics_oracledb
        fi
...
if [ -n "`which sqlplus 2>/dev/null`" ]; then
    HAS_ORACLE_CLIENT="yes"
...
if [ -z "$HAS_ORACLE_CLIENT" ]; then
    echo
    echo "*********************************************************************************************"
    echo "Exiting from script!"
    echo "ERROR: Instant Client for oracle is not installed or PATH variable does not have Oracle Path."
    echo "Initialize ORACLE_HOME with path for Oracle or LD_LIBRARY_PATH with path for Instant client."
    echo "Export these variables in /etc/bash.bashrc or bashrc of root user."
    echo "Update PATH variable by appending ORACLE_HOME and LD_LIBRARY_PATH (which ever exist) to current PATH."
    echo "Please install instant client or set PATH variable correctly. Test it by running sqlplus."
    echo "Run script again after making changes."
    echo "**********************************************************************************************"
    exit 1
fi
...
while true; do
    echo -n "Add another Oracle server? [Yn] "
    read yn
    if [ -n "`echo $yn | egrep -io '^n'`" ]; then
        break
    fi
