...
setup_database()
{
    ...
    echo
    if [ -z $USER_NAME ]; then
        echo "mongo $URL:$PORT/$DBNAME --eval \"db.runCommand({dbstats: 1})\" > /tmp/mongodb_stats.txt"
...
setup_mongodb()
{
    ...
    while [ $lc -ne 0 ]; do
      ...
      echo "Configuring first MongoDB Database (required)"
      setup_database $LABEL $URL $PORT "mongodb" "initial"
      lc=$?
    done
    ...
    while true; do
      ...
      setup_database $LABEL $URL $PORT "mongodb" ""
    done
}
