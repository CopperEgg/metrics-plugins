...
setup_admin_base_group()
{
    ...
    while true; do
        ...
        ADMIN_GROUP_NAME_VALID=$(curl -su $API_KEY:U -G --data-urlencode "name=$ADMIN_GROUP_NAME" $API_HOST/v2/revealmetrics/validate_metric_group_name?service=$SERVICE)
        ...
        if [ "$ADMIN_GROUP_NAME_VALID" == "invalid" ]; then
            ...
            ADMIN_GROUP_NAME="$METRIC_GROUP_NAME_VALID"
    ...
    while true; do
        ...
        ADMIN_DASHBOARD_NAME_VALID=$(curl -su $API_KEY:U -G --data-urlencode "name=$ADMIN_DASHBOARD" $API_HOST/v2/revealmetrics/validate_dashboard_name?service=$SERVICE)
...
setup_db_base_group()
{
    ...
    while true; do
        ...
        DB_GROUP_NAME_VALID=$(curl -su $API_KEY:U -G --data-urlencode "name=$DB_GROUP_NAME" $API_HOST/v2/revealmetrics/validate_metric_group_name?service=$SERVICE)
    ...
    while true; do
        ...
        DB_DASHBOARD_NAME_VALID=$(curl -su $API_KEY:U -G --data-urlencode "name=$DB_DASHBOARD" $API_HOST/v2/revealmetrics/validate_dashboard_name?service=$SERVICE)
